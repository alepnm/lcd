...
/* Configure the FSMC Parallel interface -------------------------------------*/
  LCD_FSMCConfig();
   ...
   // power supply setting
    ...
    LCD_WriteReg(0x0001,0x2B3F);//C,y'¶?E^a"^(3)o"??O"?320*240  0x6B3F
    LCD_WriteReg(0x0010,0x0000);
    ...
    LCD_WriteReg(0x000F,0x0000);//E'?A~e`?^(a)E^1/4µ?O"·
    ...
    LCD_WriteReg(0x004f,0);        // ??E^?O"·0
    LCD_WriteReg(0x004e,0);        // A'?E^?O"·0
...
void LCD_SetDisplayWindow(uint16_t Xpos, uint16_t Ypos, uint8_t Height, uint16_t Width)
{
  ...
  if(Xpos >= Height)
  {
    LCD_WriteReg(0x0044, (0x00FF&(Xpos - Height + 1))|(Xpos<<8));
  }
  else
  {
    LCD_WriteReg(0x0044, Xpos<<8);
  }
...
{
  /* Display On */
  LCD_WriteReg(LCD_REG_7, 0x0033); /* 262K color and display ON */
}
