...
void Draw_Image8bit(uint16_t x, uint16_t y, uint16_t x_res, uint16_t y_res, const uint8_t *ptr_image)
{
	uint16_t color;
  uint16_t i = 0, j = 0;
...
//void Draw_Image(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t *pic)
//{
...
//        {
//					Draw_Pixel(x_index, y_index, pic[pic_index]);				
//					pic_index++;
//				}
//		} 
//}

//void Draw_Image8bit(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint8_t *pic)
//{
//	uint16_t color;
//	uint16_t x_index, y_index;
//	uint32_t pic_index = 0;
...
void LCD_write_comand(uint16_t reg) 
  {
    LCD->Register  = reg;
  }
...
void LCD_Init(void)
  ...
  Delay_ms(10);
    ...
    //LCD_write_comand(0x35); // tearing effect on
		
		LCD_write_comand(0x36);		//rotate
		LCD_write_data   (0x58);	//0x28, 0x58, 0x88, 0xE8
		
    //LCD_write_comand(0xb4); // display inversion
